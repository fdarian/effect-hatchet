...
- `schedule.list` makes exactly one call and returns one page — it does not accumulate every schedule for you. The result is `{ schedules, pagination? }`: `schedules` is that page's `ScheduledRun[]`, and `pagination` (when the source reports it) is `{ currentPage?, nextPage?, numPages? }`, all 1-indexed. `nextPage` is **not** an end-of-results sentinel — the server always populates it, clamping it back to `currentPage` on the last page instead of omitting it, so looping `while (pagination.nextPage is defined)` will hang forever. Walk every page by advancing `offset` to `(pagination.nextPage - 1) * limit` while `pagination.nextPage > pagination.currentPage`, and also stop once a page comes back with an empty `schedules` array — the zero-result case never satisfies the clamp, since `numPages` is `0` and `nextPage` just keeps counting up on every call. `statuses` filters by `ScheduledRunStatus` (`PENDING`, `RUNNING`, `SUCCEEDED`, `FAILED`, `CANCELLED`, `QUEUED`, `SCHEDULED`); results are ordered by `triggerAt`.
...
- Testing gotcha: inside an `it.layer(...)` block (the `@effect/vitest` fixture that builds a layer once and shares it across every test in the block), writing `Effect.provide(Hatchet.layerInMemory())` inside one of those tests does **not** get you a fresh instance — it's the same `Layer` reference `it.layer` already built, so Effect's layer memoization silently reuses the shared one. Symptom: schedule/cron counts come out too high, because schedules created by earlier tests are still sitting in what looks like a fresh instance. Fix: wrap it in `Layer.fresh(Hatchet.layerInMemory())` (from `effect`'s `Layer` module) when a test needs a genuinely isolated instance.
